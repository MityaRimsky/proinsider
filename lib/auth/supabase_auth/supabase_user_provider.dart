...
class ProinsiderSupabaseUser extends BaseAuthUser {
  ProinsiderSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> proinsiderSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ProinsiderSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
